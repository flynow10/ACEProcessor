...
memalign(size_t align, size_t sz)
{
       ...
       nextfree = &(*nextfree)->next)
  {
    ...
    if (!*nextfree)
    {
      ...
      if (MALLOC_DIRECTION < 0)
      {
        old_size += M_ALIGN_SUB(((size_t)__malloc_end - old_size + sizeof(size_t)),
                                align);
 ...
 enlarge it.  */
    if (!block->next && old_size < real_size + before_size && __malloc_end == (void *)((size_t)block + block->size))
    {
      if (MALLOC_DIRECTION < 0)
      {
        size_t moresize = real_size - block->size;
    ...
    if (old_size >= real_size + before_size)
    {
      ...
      if (before_size != 0)
      {
        ...
        if (old_size - before_size <= real_size + sizeof(struct freelist_entry))
        {
